...
class EvalCallback():
    ...
    def on_epoch_end(self, epoch, model_eval):
        if epoch % self.period == 0 and self.eval_flag:
            ...
            print("Calculate miou, mPA.")
            _, IoUs, PA_Recall, _ = compute_mIoU(gt_dir, pred_dir, self.image_ids, self.num_classes, None)  # 执行计算mIoU的函数
            ...
            # 绘制mpa曲线
            plt.figure()
            plt.plot(self.epoches, self.mpas, 'blue', linewidth = 2, label='train mPA')

            plt.grid(True)
            plt.xlabel('Epoch')
            plt.ylabel('mPA')
            plt.title('A mPA Curve')
            plt.legend(loc="upper right")

            plt.savefig(os.path.join(self.log_dir, "epoch_mpa.png"))
            plt.cla()
            plt.close("all")
