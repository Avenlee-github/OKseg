...
from utils.utils_decent import Decentration_Cal

from utils.hgnet.model import KFSGNet
from utils.hgnet.center_predict import predict
...
class SegFormer_Segmentation(object):
    ...
    def generate(self, onnx=False):
        ...
        #-------------------------------#
        self.hgnet = KFSGNet()
        self.hgnet.load_state_dict(torch.load("utils/hgnet/model.ckpt", map_location=device))
        self.hgnet = self.hgnet.float().to(device)
        self.hgnet = self.hgnet.eval()
        print(f"Center point detect model loaded.")
